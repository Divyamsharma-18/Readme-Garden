    ...
    try {
      if (process.env.OPENAI_API_KEY) {
        const result = await generateText({
          ...
          IMPORTANT: Create a COMPLETELY NEW VERSION of this README. Don't just modify it - write it from scratch while keeping the same information.
          Make sure it strongly reflects the ${vibe} style in tone, structure, and formatting.
          Return ONLY the rewritten README content without any code block formatting or explanations.
          `,
          maxTokens: 2500,
          temperature: 0.9,
        })
